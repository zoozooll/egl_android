...
void GLrenderS::loadTexture() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, 1024, 64, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE,
                 sb_data);
...
void GLrenderS::SurfaceCreate() {
    ...
    glBufferData(GL_ARRAY_BUFFER, 4 * POINTS * sizeof(GLfloat), gScreenVertices,
                 GL_STATIC_DRAW); //扇形坐标
    glEnableVertexAttribArray(aPositionLocation);
    glBindBuffer(GL_ARRAY_BUFFER, texture_buffer_id[1]);
    glBufferData(GL_ARRAY_BUFFER, 4 * POINTS * sizeof(GLfloat), gTextureVertices,
                 GL_STATIC_DRAW);//矩形坐标
...
void GLrenderS::DrawFrame() {
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 1000, 49, GL_LUMINANCE, GL_UNSIGNED_BYTE,
                    sb_data);  //更新显示图像
